...
class ConfigAnnotationProcessor(
	...
	override fun process(resolver: Resolver): List<KSAnnotated> {
		resolver.getSymbolsWithAnnotation("moe.nea.firmod.util.data.Config")
	...
	override fun finish() {
		...
		val compatFile =
			codeGenerator.createNewFile(
				dependencies,
				"moe.nea.firmod.annotations.generated.$sourceSetName",
				generatedFileName
			)
				.bufferedWriter()
		...
		compatFile.appendLine("class $generatedFileName : moe.nea.firmod.util.data.IConfigProvider {")
		compatFile.appendLine(
			"""
	override val configs: List<moe.nea.firmod.util.data.IDataHolder<*>> = listOf(${configs.joinToString { it.first.asString() }})
	"""
		)
		compatFile.appendLine("}")
		compatFile.close()
		val metaInf = codeGenerator.createNewFileByPath(
			dependencies,
			"META-INF/services/moe.nea.firmod.util.data.IConfigProvider", extensionName = ""
		)
			.bufferedWriter()
