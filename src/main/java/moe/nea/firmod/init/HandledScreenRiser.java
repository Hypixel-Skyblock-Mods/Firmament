...
import moe.nea.firmod.mixins.ScreenInputEvents;
import moe.nea.firmod.mixins.customgui.PatchGenericScreen;
...
public class HandledScreenRiser extends RiserUtils {
	...
	/// @see PatchGenericScreen#mouseScrolled_firmod
	void addMouseScroll(ClassNode classNode) {
		addSuperInjector(
			classNode, mouseScrolled.mapped(), mouseScrolled.mappedDesc(), Screen, AbstractContainerEventHandler, "mouseScrolled_firmod",
			insns -> {
				// ALOAD 0, load this
	...
	/// @see PatchGenericScreen#keyReleased_firmod
	void addKeyReleased(ClassNode classNode) {
		addSuperInjector(
			classNode, keyReleased.mapped(), keyReleased.mappedDesc(), Screen, AbstractContainerEventHandler, "keyReleased_firmod",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// ALOAD 1, load args
	...
	/// @see PatchGenericScreen#mouseReleased_firmod
	void addMouseReleased(ClassNode classNode) {
		addSuperInjector(
			classNode, mouseReleased.mapped(), mouseReleased.mappedDesc(), Screen, AbstractContainerEventHandler, "mouseReleased_firmod",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// ALOAD 1, load args
	...
	/// @see PatchGenericScreen#mouseDragged_firmod
	void addMouseDragged(ClassNode classNode) {
		addSuperInjector(
			classNode, mouseDragged.mapped(), mouseDragged.mappedDesc(), Screen, AbstractContainerEventHandler, "mouseDragged_firmod",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// ALOAD 1 load event
	...
	/// @see PatchGenericScreen#mouseClicked_firmod_customGui
	/// @see ScreenInputEvents#onMouseClicked_firmod_generic
	void addMouseClicked(ClassNode classNode) {
		...
		addSuperInjector(
			...
			Screen, AbstractContainerEventHandler, "onMouseClicked_firmod_generic", loadInsns);
		addSuperInjector(
			classNode, mouseClickedScreen.mapped(), mouseClickedScreen.mappedDesc(),
			Screen, AbstractContainerEventHandler, "mouseClicked_firmod_customGui", loadInsns);
	}

	/// @see PatchGenericScreen#charTyped_firmod
	void addCharTyped(ClassNode classNode) {
		addSuperInjector(
			classNode, charTyped.mapped(), charTyped.mappedDesc(),
			Screen, AbstractContainerEventHandler, "charTyped_firmod",
			insns -> {
				// ALOAD 0, load this
				insns.add(new VarInsnNode(Opcodes.ALOAD, 0));
				// ALOAD 1, load args
