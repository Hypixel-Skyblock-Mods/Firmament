...
import moe.nea.firmod.gui.config.GuiAppender
import moe.nea.firmod.gui.config.HudMeta
import moe.nea.firmod.gui.config.HudMetaHandler
import moe.nea.firmod.gui.config.HudPosition
...
abstract class ManagedConfig(
	...
	enum class Category {
		...
		val labelText: Component = Component.translatable("firmod.config.category.${name.lowercase()}")
		val description: Component = Component.translatable("firmod.config.category.${name.lowercase()}.description")
...
//	}
	...
	protected fun position(
		...
		val label = Component.translatable("firmod.config.${name}.${propertyName}")
		return option(propertyName, {
			val p = default()
			HudMeta(HudPosition(p.x(), p.y(), 1F), Firmod.identifier(propertyName), label, width, height)
	...
	protected fun keyBindingWithOutDefaultModifiers(
		...
		return option(propertyName, default, KeyBindingHandler("firmod.config.${name}.${propertyName}", this))
	}
	...
	val translationKey get() = "firmod.config.${name}"
