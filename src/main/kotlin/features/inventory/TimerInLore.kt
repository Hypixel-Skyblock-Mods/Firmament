...
object TimerInLore {
	...
	enum class CountdownTypes(
		...
		SPECIAL_MAYOR("Time until:", "Occurs at"),
		;
	}
	...
	fun creationInLore(event: ItemTooltipEvent) {
		...
		event.lines.add(tr("firmod.lore.creationtimestamp", "Created at: $formattedTimestamp").grey())
	}
			...
			if (countdownType.isRelative) {
				if (lastTimer == null) {
					event.lines.add(
						i + 1,
						tr(
							"firmod.loretimer.missingrelative",
							"Found a relative countdown with no baseline (Firmod)"
						).grey()
					)
					continue
				}
