...
object HeadModelChooser {
	...
	interface HasExplicitHeadModelMarker {
		fun markExplicitHead_Firmod()
		fun isExplicitHeadModel_Firmod(): Boolean
		...
		override fun update(
			...
			val instance =
				if (IS_CHOOSING_HEAD_MODEL.get()) {
					HasExplicitHeadModelMarker.cast(state).markExplicitHead_Firmod()
					head
				} else {
					regular
				}
	...
	data class Unbaked(
		...
		companion object {
			...
			fun fromLegacyJson(jsonObject: JsonObject, unbakedModel: ItemModel.Unbaked): ItemModel.Unbaked {
				val model = jsonObject["firmod:head_model"] ?: return unbakedModel
