...
import moe.nea.firmod.features.texturepack.CustomBlockTextures.createBakedModels
import moe.nea.firmod.features.texturepack.CustomGlobalTextures.logger
...
import moe.nea.firmod.util.json.BlockPosSerializer
...
object CustomBlockTextures {
	...
	private val sodiumReloadTask = runCatching {
		val r = Class.forName("moe.nea.firmod.compat.sodium.SodiumChunkReloader")
			.getConstructor()
	...
	private fun prepare(manager: ResourceManager): BakedReplacements {
		...
		for ((file, resource) in resources) {
			val json =
				Firmod.tryDecodeJsonFromStream<CustomBlockOverride>(resource.open())
					.getOrElse { ex ->
	...
	/**
	 * Used by [moe.nea.firmod.init.SectionBuilderRiser]
	 */
